...
react-icons
jwt-decode
three                            # 3D rendering (WebGL) for interactive car viewer
meshoptimizer                    # Mesh compression decoder for .glb models
```
...
- **Auction detail media carousel** — `ImageCarousel` component supports mixed image and 3D slide types with swipe/drag gestures, keyboard navigation, prev/next arrows, and dot indicators. For SU7/Praga/Mazzanti listings, the carousel includes an interactive Three.js 3D car viewer (`Su7ThreeHero`, lazy-loaded) alongside image angle variants; non-3D listings show a single image slide. The 3D viewer loads compressed `.glb` models with meshopt decoding, studio-grade lighting, a reflective floor, orbit controls, and a compact right-side exterior paint swatch selector with smooth color transitions. `Su7ThreeHero` uses model-specific config (asset path, root alignment, wheel spin axis/direction, intro zoom scale, audio profile) to normalize behavior across different vehicle rigs, and applies a fallback paint-material detection path for rigs whose exterior material names are inconsistent. The carousel also supports 3D-focused full-tab expansion mode with dedicated expand/minimize controls and Escape-key handling that does not conflict with modal close behavior.
- **Auction card 3D previews** — For recognized 3D model listings (SU7/Praga/Mazzanti), browse cards render a live `Su7ThreeHero` preview directly in the card media area (lazy-loaded). Card previews are intentionally non-interactive (`pointer-events: none`) and use `animateWheels={false}` to keep wheel meshes static and reduce motion noise in list view. Card navigation sets `openMedia: "3d"` in route state so detail pages can open the carousel on the 3D slide first. A low-detail browse mode can force static "3D preview available" placeholders (instead of live WebGL previews) once several 3D cards are present on the same page, and also removes animated glow effects from countdown/category chips to reduce graphics load.
...
**Frontend:** “GM tools” opens from a **right-edge tab** (admins on any page); **slides in from the right** as a full-height panel with backdrop dismiss and error toasts. The **Seed auctions** tab combines **random/synthetic** and **GT7 manifest** sources behind one form. **Categories** tab: reload tree from `GET /api/categories`, create/update/delete via GM category endpoints (string keys + lucide icon keys), and emit an in-app auction-list refresh signal so open browse/search surfaces and create flows refetch category data without a full page reload. The General tab includes a persisted **low-detail mode** toggle for auction browse cards (stored in `localStorage` and propagated via an in-app window event so open pages react immediately). **Delete all auctions** is available in the panel for full environment reset alongside other GM actions. Legacy `/admin/gm` redirects to `/admin`.
