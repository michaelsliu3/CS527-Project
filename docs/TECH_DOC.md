...
- **Auction detail media carousel** — `ImageCarousel` component supports mixed image and 3D slide types with swipe/drag gestures, keyboard navigation, prev/next arrows, and dot indicators. For SU7 listings, the carousel includes an interactive Three.js 3D car viewer (`Su7ThreeHero`, lazy-loaded) alongside image angle variants; non-SU7 listings show a single image slide. The 3D viewer loads a compressed `.glb` model with meshopt decoding, studio-grade lighting, a reflective floor, orbit controls, and a compact right-side exterior paint swatch selector with smooth color transitions.
